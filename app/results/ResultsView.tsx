...
function statusTone(status: string): "yellow" | "green" | "neutral" {
...
function roundSummary(round: GameResultsRound) {
  ...
  const wordTotal = round.reviews.reduce(
    (total, review) => total + wordCount(review.reviewText),
    0
  );
...
export function ResultsView({ data }: Props) {
  const { viewerId, email, viewerDisplayName, group, game, roster, rounds } =
    data;
    ...
    roundStats.length > 0
      ? roundStats.reduce(
          (best, current) => {
            ...
            if (!best || best.avg == null || current.avg > best.avg)
              return current;
            return best;
          },
  ...
  const totalReviews = rounds.reduce(
    (total, round) => total + round.reviews.length,
    0
  );
  const totalWords = rounds.reduce(
    (total, round) =>
      total +
      round.reviews.reduce(
        (roundTotal, review) => roundTotal + wordCount(review.reviewText),
        0
      ),
    0
  );
  ...
  const scoreboard = roster
    .map((member) => {
      const pickedRounds = roundStats.filter(
        (entry) => entry.round.pickerId === member.userId
      );
      ...
      const averagePickedScore =
        pickedAverages.length > 0
          ? Math.round(
              (pickedAverages.reduce((total, score) => total + score, 0) /
                pickedAverages.length) *
                10
            ) / 10
          : null;
      const reviewsWritten = rounds
        .flatMap((round) => round.reviews)
        .filter((review) => review.userId === member.userId);
      const averageGivenScore =
        reviewsWritten.length > 0
          ? Math.round(
              (reviewsWritten.reduce(
      ...
      return {
        ...
        averageGivenScore,
        topPickCount: pickedRounds.filter(
          (entry) => bestRound?.round.id === entry.round.id
        ).length,
      };
    })
    .sort((left, right) => {
      ...
      if (right.topPickCount !== left.topPickCount)
        return right.topPickCount - left.topPickCount;
      if (right.pickedRounds !== left.pickedRounds)
        return right.pickedRounds - left.pickedRounds;
  ...
  const clubAverage =
    averageAcrossRounds.length > 0
      ? Math.round(
          (averageAcrossRounds.reduce((total, score) => total + score, 0) /
            averageAcrossRounds.length) *
            10
        ) / 10
      : null;
  const shareSummary =
    rounds.length > 0 ? buildResultsShareSummary(data) : null;
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <span className="inline-flex items-center rounded-full border-2 border-foreground/85 bg-surface/85 px-3 py-1 text-xs font-bold text-foreground-secondary">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <StatTile
              label="Current game"
              value={game ? statusLabel(game.status) : "No game"}
            />
            <StatTile
              label="Round"
              value={game ? `${game.currentRound} / ${game.maxRounds}` : "--"}
            />
            <StatTile
              label="Latest reveal"
              value={latestRound?.round.albumName ?? "None yet"}
            />
            <StatTile
              label="Best round"
              value={bestRound?.round.albumName ?? "Waiting"}
            />
          </div>

          <div className="flex flex-wrap gap-3">
            <Link href="/play" className={primaryButtonClass}>
              Back to Play
            </Link>
      ...
      {!group ? (
        <section className="rounded-[2rem] border-2 border-foreground bg-accent-pink/30 p-6 landing-sticker">
      ...
      {group && !game ? (
        <section className="rounded-[2rem] border-2 border-foreground bg-accent-lime/30 p-6 landing-sticker">
      ...
      {group && game ? (
        <>
          <section className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]">
            <div className="rounded-[2rem] border-2 border-foreground bg-accent-peach/35 p-6 landing-sticker">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
              <div className="rounded-[1.75rem] border-2 border-foreground bg-accent-pink/30 p-5 landing-sticker">
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-accent-pink-fg">
                  Latest reveal
                </p>
              ...
              <div className="rounded-[1.75rem] border-2 border-foreground bg-accent-lime/30 p-5 landing-sticker">
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-accent-lime-fg">
                  This game
                </p>
                <h3 className="mt-2 text-lg font-black tracking-tight">
          ...
          {rounds.length === 0 ? (
            <section className="rounded-[2rem] border-2 border-foreground bg-surface p-6 landing-sticker">
              ...
              <p className="mt-3 max-w-prose text-sm leading-7 text-foreground-secondary">
                Once a round closes and the room reveals scores, it will appear
                here with written notes, averages, and the running table.
              </p>
            </section>
          ) : (
            <>
              {shareSummary ? (
                <CopyResultsSummaryButton summary={shareSummary} />
              ) : null}
                  ...
                  {roundStats.map((entry) => {
                    ...
                    return (
                      <li
                        key={round.id}
                        className={cx(
                          "rounded-[2rem] border-2 border-foreground p-6 landing-sticker",
                          isBest ? "bg-accent-yellow/30" : "bg-surface"
                        )}
                      >
                        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                          ...
                          <div className="grid gap-3 sm:grid-cols-3 lg:w-[27rem] lg:grid-cols-1 xl:grid-cols-3">
                            ...
                            <StatTile label="Words" value={wordTotal} />
                          </div>
                        </div>
                          ...
                          <ul className="mt-5 grid gap-3">
                            {round.reviews.map((review) => (
                              ...
                              >
                                ...
                                <div className="mt-3 h-2 w-full overflow-hidden rounded-full border-2 border-foreground/15 bg-surface">
                                  <div
                                    className={cx(
                                      "h-full",
                                      scoreBarClass(review.rating)
                                    )}
                                    style={{
                                      width: `${Math.max(0, Math.min(100, review.rating * 10))}%`,
                                    }}
                                  />
                                </div>
          ...
          <section className="grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
            <div className="rounded-[2rem] border-2 border-foreground bg-surface p-6 landing-sticker sm:p-7">
              <div className="flex items-end justify-between gap-4">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-foreground-secondary">
                    Club record
                ...
                <ul className="mt-5 grid gap-3">
                  {scoreboard.map((entry, index) => (
                    <li
                      key={entry.userId}
                      className={cx(
                        "rounded-2xl border-2 border-foreground/10 p-4",
                        index === 0 && entry.averagePickedScore != null
                          ? "border-foreground bg-accent-yellow/30 landing-sticker-sm"
                    ...
                    >
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2 text-xs">
                            <span className="inline-flex items-center rounded-full border-2 border-foreground/15 bg-surface px-3 py-1 font-bold text-foreground-secondary">
                              {entry.pickedRounds} pick
                              {entry.pickedRounds === 1 ? "" : "s"}
                            </span>
                            <span className="inline-flex items-center rounded-full border-2 border-foreground/15 bg-surface px-3 py-1 font-bold text-foreground-secondary">
                              {entry.reviewsWritten} review
                              {entry.reviewsWritten === 1 ? "" : "s"}
                            </span>
                            {entry.topPickCount > 0 ? (
                              <span className={toneBadgeClass("lime")}>
                        ...
                        <div className="grid gap-2 text-right">
                          ...
                          <p className="text-xs text-foreground-secondary">
                            Gives{" "}
                            {entry.averageGivenScore != null
                              ? entry.averageGivenScore.toFixed(1)
                              : "--"}{" "}
                            on average
                          </p>
                        </div>
            ...
            <div className="grid gap-4">
              <div className="rounded-[2rem] border-2 border-foreground bg-accent-yellow/30 p-6 landing-sticker">
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-accent-yellow-fg">
                  Club stats
                </p>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <StatTile
                    label="Club average"
                    value={clubAverage != null ? clubAverage.toFixed(1) : "--"}
                  />
                  <StatTile label="Words written" value={totalWords} />
                </div>
              </div>

              <div className="rounded-[2rem] border-2 border-foreground bg-accent-lime/30 p-6 landing-sticker">
                <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-accent-lime-fg">
                  Best round
                </p>
                <h3 className="mt-2 text-lg font-black tracking-tight">
